...
class Encomienda(models.Model):
    ...
    sucursal_actual = models.ForeignKey(
        'sucursales.Sucursal', on_delete=models.PROTECT,
        related_name='encomiendas_actuales',
        null=True, blank=True
    )
    ...
    def calcular_costo(self):
        """Calcula el costo del envío basándose en precio base y peso"""
        PRECIO_POR_KG_EXTRA = 2.50
        PESO_BASE = 5.0
        costo = self.ruta.precio_base
        if self.peso_kg > PESO_BASE:
            from decimal import Decimal
            costo += (self.peso_kg - Decimal(str(PESO_BASE))) * Decimal(str(PRECIO_POR_KG_EXTRA))
        return round(costo, 2)

    @classmethod
    def crear_con_costo_calculado(cls, remitente, destinatario, ruta, empleado, descripcion, peso_kg, **kwargs):
        """Fábrica para crear encomiendas con costo automático"""
        import uuid
        from datetime import timedelta
        # Formato: ENC-YYYYMMDD-HEX6
